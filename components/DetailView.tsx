...
const generateBreakdownData = (itemId: string) => {
  const isHours = itemId.includes('hours') || itemId.includes('overtime');
  const scale = isHours ? 1000000 : 1000; 
  const total = Math.floor(Math.random() * 50 * scale) + (20 * scale);
  const target = total * 0.95; // 5% below actual for target example

  const zones = [
    { name: 'Calgary', value: Math.floor(total * 0.38) },
    { name: 'Edmonton', value: Math.floor(total * 0.35) },
    { name: 'Central', value: Math.floor(total * 0.12) },
    { name: 'North', value: Math.floor(total * 0.09) },
    { name: 'South', value: Math.floor(total * 0.06) },
  ];
...
export const DetailView: React.FC<DetailViewProps> = ({ item, onBack, isDarkMode }) => {
  ...
  const data = useMemo(() => generateBreakdownData(item.id), [item.id]);
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="hidden sm:flex gap-3">
            <button 
               ...
               className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 hover:border-blue-300 dark:hover:border-blue-700 transition-all shadow-sm"
            >
      ...
      >
         ...
         <div className="space-y-6">
            <FilterGroup icon={Calendar} label="Fiscal Year">
              <select className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 text-sm rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 transition-all">
            ...
            <FilterGroup icon={MapPin} label="Zone">
              <div className="grid grid-cols-2 gap-2">
                 {['North', 'Edmonton', 'Central', 'Calgary', 'South'].map(z => (
                    <button key={z} className="text-xs font-medium px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-600 transition-colors text-left text-slate-600 dark:text-slate-300">
                       {z}
                    </button>
                 ))}
              </div>
            ...
            <FilterGroup icon={Building2} label="Functional Center">
               <input 
                  type="text" 
                  placeholder="Search..." 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-sm rounded-xl p-3 outline-none" 
               />
            </FilterGroup>
